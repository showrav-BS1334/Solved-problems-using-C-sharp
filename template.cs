...
namespace CP
{
    class Solution
    {
        ...
        public void Solve()
        {
            ...
            var dq = new LinkedList<long>();

            for (int i = 0; i < n; i++)
            {
                if (dq.Count == 0) dq.AddLast(a[i]);
                else
                {
                    if(dq.First() < a[i]) dq.AddLast(a[i]);
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            using (StreamReader reader = new StreamReader("input.txt")) // input from file
            {
