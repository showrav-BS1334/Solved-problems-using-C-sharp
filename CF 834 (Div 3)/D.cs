...
namespace CP
{
    class Solution
    {
        ...
        public void Solve()
        {
            ...
            long n = list[0], m = list[1];
            bool isPrime = true;
            foreach (var x in allPrimes)
            {
                if (n % x == 0)
                {
                    isPrime = false;
            ...
            if (isPrime) a.Add(n);
            else a = FindPrimeFactors(n);

            long cnt2 = a.Count(x => x == 2);
            long cnt5 = a.Count(y => y == 5);
            long n2 = 0, n5 = 0;
            ...
            if (cnt2 > cnt5)
            {
                ...
                long y = (n % 10) * (x % 10);
                if (y % 10 != 0) Console.WriteLine(n * m);
