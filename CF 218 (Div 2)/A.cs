...
namespace CP
{
    class Solution
    {
        ...
        public void Solve()
        {
            ...
            long haveB = list[0], haveS = list[1], haveC = list[2];
            list = ScanLongList();
            long costB = list[0], costS = list[1], costC = list[2];
            long money = ScanLongList()[0];

            long B = s.Count(c => c == 'B');
            long S = s.Count(c => c == 'S');
            long C = s.Count(c => c == 'C');

            if (B == 0) costB = 0;
            if (S == 0) costS = 0;
            if (C == 0) costC = 0;

            long L = 0, R = 100000L * 100000 * 100000;
            while (L < R)
            {
                long mid = L + (R - L + 1) / 2;
                long needB = (mid * B - haveB) * costB;
                long needS = (mid * S - haveS) * costS;
                long needC = (mid * C - haveC) * costC;

                if (needB < 0) needB = 0;
                if (needS < 0) needS = 0;
                if (needC < 0) needC = 0;

                if (needS + needC + needB > money) R = mid - 1;
                else L = mid;
            }
