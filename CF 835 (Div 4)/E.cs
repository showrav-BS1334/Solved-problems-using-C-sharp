...
namespace CP
{
    class Solution
    {
        ...
        {
            int n = ScanIntList()[0];
            List<long> tem = ScanLongList();

            List<long> a = Enumerable.Repeat((long)0, n + 5).ToList();
            List<long> zero = Enumerable.Repeat((long)0, n + 5).ToList();
            List<long> one = Enumerable.Repeat((long)0, n + 5).ToList();
